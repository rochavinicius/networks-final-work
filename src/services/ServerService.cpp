#include "../../headers/services/ServerService.h"

// polinomio: 1000 1011 (bin), 0x8C (hexa), 139 (dec)
#define CRC_POLYNOME 139

unsigned char const crc8x_table[] = {
    };

unsigned crc8x_fast(unsigned crc, void const *mem, size_t len) {
    unsigned char const *data = mem;
    if (data == NULL)
        return 0xff;
    crc &= 0xff;
    while (len--)
        crc = crc8x_table[crc ^ *data++];
    return crc;
}

ServerService::ServerService(IFlowControlUnit flowControl)
    : IBaseService(flowControl)
{
}

ServerService::~ServerService()
{
}

void ServerService::Start()
{
    int serverSocket;
    int newSocket;
    // char buffer[MAX_BUFFER_SIZE];
    char inputData[MAX_BUFFER_SIZE];
    std::string buffer;
    struct sockaddr_in server_addr;
    struct sockaddr_in client_addr;

    std::cout << "Starting server service..." << std::endl;

    memset(&buffer, '\0', sizeof(buffer));

    // Creating socket file descriptor
    if ((serverSocket = socket(AF_INET, SOCK_DGRAM, 0)) < 0)
    {
        std::cerr << "Server socket creation failed." << std::endl;
        exit(EXIT_FAILURE);
    }

    std::cout << "Server socket created." << std::endl;

    // Filling server information
    server_addr.sin_family = AF_INET; // IPv4
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(SERVER_PORT);

    // Bind the socket with the server address
    if (bind(serverSocket, (const struct sockaddr *)&server_addr,
             sizeof(server_addr)) < 0)
    {
        std::cerr << "Server bind failed." << std::endl;
        exit(EXIT_FAILURE);
    }

    std::cout << "Server binded on port: " << SERVER_PORT << "." << std::endl;
    std::cout << "Server started. Waiting for incoming connections..." << std::endl;

    int c = sizeof(struct sockaddr_in);
    while ((newSocket = recvfrom(serverSocket, inputData, sizeof(inputData), MSG_WAITALL,
                                 (struct sockaddr *)&client_addr, (socklen_t *)&c)) >= 0)
    {
        std::cout << "Connection accepted" << std::endl;

        // ******************* PACOTE INICIAL *******************

        // para o pacote de estabelecimento de conexao, nao utiliza controle de fluxo
        // apenas dropa a mensagem caso esteja com erro
        
        // verificar falhas pacote inicial
        unsigned int crcRet = crc8x_fast(CRC_POLYNOME, buffer.c_str(), sizeof(buffer));
        if (crcRet != 0)
        {
            // limpa o buffer para nao deixar sujeira
            buffer.clear();
            // memset(&buffer, '\0', sizeof(buffer));
            continue;
        }

        // EU TO AKI

        // Parse package

        // do
        // {
        //     crcRet = crc8x_fast(CRC_POLYNOME, buffer.c_str(), sizeof(buffer));
        //     sendto(serverSocket, message.c_str(), message.length(), MSG_CONFIRM,
        //                  (const struct sockaddr *)&server_addr,
        //                  sizeof(server_addr));
        //     // recvfrom();
        // } while (crcRet != 0);

        // pacote inicial nao esta no tamanho requirido, entao discarta
        if (buffer.size() != 9) 
        {
            // limpa o buffer para nao deixar sujeira
            buffer.clear();
            // memset(&buffer, '\0', sizeof(buffer));
            continue;
        }

        int flowControl = atoi(buffer.substr(0, 4).c_str());
        int windowSize = atoi(buffer.substr(4, 1).c_str());
        int fileSize = atoi(buffer.substr(5, 4).c_str());

        if (flowControl != 0 && flowControl != 1)
        {
            // limpa o buffer para nao deixar sujeira
            buffer.clear();
            // memset(&buffer, '\0', sizeof(buffer));
            continue;
        }
        if (windowSize < 1 || windowSize > 16)
        {
            buffer.clear();
            continue;
        }

        auto flowControlType = flowControl == 1 ? GO_BACK_N : STOP_AND_WAIT;


        /*pthread_t handlerThread;
        Client client(this->m_clients.size(), newSocket);
        
        //TODO: add mutex to manipulate list
        this->m_clients.push_back(client);

        if (pthread_create(&handlerThread, NULL, &this->HandlerThread,
                           (void *)&this->m_clients[this->m_clients.size() - 1]) < 0)
        {
            std::cerr << "Could not create connection handler thread." << std::endl;
            exit(EXIT_FAILURE);
        }

        std::cout << "Client nr " << this->m_clients.size() << " assigned to handler thread." << std::endl;
        */
    }

    if (newSocket < 0)
    {
        std::cerr << "Error recvfrom server." << std::endl;
        exit(EXIT_FAILURE);
    }
}

/*void ServerService::HandlerThread(void *ptr)
{
    Client *client = (Client *)ptr;
    //TODO: handle client using protocol
    // sendto(sockfd, (const char *)hello, strlen(hello),
    //            MSG_CONFIRM, (const struct sockaddr *)&cliaddr,
    //            len);

    std::string buffer


    recvfrom(client->GetSocket(), buffer.c_str(), MAX_BUFFER_SIZE,  
                0, ( struct sockaddr *) &cliaddr, &len); 
}*/